...
async def test_async_with_callback():
    async_face_study = AsyncFaceStudy(analyze_type='emotion', window='Async With Callback')
    await async_face_study.run()

# Função de teste para a versão assíncrona sem callback
async def test_async_without_callback():
    async_face_study = AsyncFaceStudy(analyze_type='emotion', window='Async Without Callback')
    await async_face_study.run()
